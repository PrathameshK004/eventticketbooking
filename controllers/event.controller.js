    ...
    Event.find({ isTemp: false, isLive: true })
        .then(async (events) => {
            const updatedEvents = await Promise.all(events.map(async (event) => {
                ...
                const user = await User.findById(event.userId).select("eventRating");

                return {
                    ...event.toObject(),
                    isNew,
                    eventRating: user ? user.eventRating : 0 // Get eventRating if user exists
                };
            }));
...
async function getAllEventsByFilter(req, res) {
    try {
        ...
        // Get organizer IDs
        const organizerIds = events.map(event => event.userId);

        // Fetch organizer ratings
        const organizers = await User.find({ _id: { $in: organizerIds } }).select("eventRating");

        // Map organizer ratings to their IDs
        const organizerRatings = organizers.reduce((acc, organizer) => {
            acc[organizer._id] = organizer.eventRating;
            return acc;
        }, {});

        // Add isNew attribute and eventRating to each event
        const updatedEvents = events.map(event => {
            ...
            return {
                ...event.toObject(),
                isNew,
                eventRating: organizerRatings[event.userId] || null  // Attach eventRating
            };
        });
...
async function getEventById(req, res) {
    ...
    try {
        ...
        // Find the organizer (user) using the stored userId in event
        const organizer = await User.findById(event.userId).select("eventRating");

        // Attach eventRating to the response
        const eventWithRating = {
            ...event.toObject(),
            eventRating: organizer ? organizer.eventRating : null // Set null if organizer not found
        };

        res.status(200).json(eventWithRating);
    } catch (err) {
